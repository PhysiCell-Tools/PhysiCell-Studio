#
# anim_cells_tracks.py:  render/animate PhysiCell .svg files (using left/right arrows on keyboard), showing cells and their tracks.
#
# Usage:
#  python anim_cells_tracks.py <show_nucleus start_index axes_min axes_max>
...
#  python anim_cells_tracks.py 
#  python anim_cells_tracks.py 0 5 700 1300 
#
...
trackd={}   # dictionary to hold all (x,y) positions of cells
...
#      print("type(s)=",type(s))
      ...
      if (child.attrib['id'] in trackd.keys()):
        trackd[child.attrib['id']] = np.vstack((trackd[child.attrib['id']], [ xval, yval ]))
      else:
        trackd[child.attrib['id']] = np.array( [ xval, yval ])
...
#  circles(xvals,yvals, s=rvals, c=rgbs)
  ...
  for key in trackd.keys():
    if (len(trackd[key].shape) == 2):
      x = trackd[key][:,0]
      y = trackd[key][:,1]
      plt.plot(x,y)
