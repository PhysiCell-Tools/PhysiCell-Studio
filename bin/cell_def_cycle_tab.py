...
class CycleTab(CellDefSubTab):
    ...
    def update_division_function_params(self):
        print(f"update_division_function_params: {self.get_current_celldef()}")
        cdname = self.get_current_celldef()
    ...
    def fill_xml(self,pheno,cdef):
        ...
        # asymmetric division
        asym_div = ET.SubElement(pheno, "cell_asymmetric_divisions")
        asym_div_weights = ET.SubElement(asym_div, "asymmetric_division_weights")
        for other_cdname in self.param_d[cdef]['asymmetric_division_weight'].keys():
            asym_div_weight = ET.SubElement(asym_div_weights, "asymmetric_division_weight", {"name":other_cdname, "units":"dimensionless"})
            asym_div_weight.text = self.param_d[cdef]['asymmetric_division_weight'][other_cdname]
