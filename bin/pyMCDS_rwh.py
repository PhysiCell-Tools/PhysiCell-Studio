...
class pyMCDS:
    ...
    def get_mesh_spacing(self):
        ...
        mesh's spatial units. Assumes that voxel centers fall on integer values. (rwh: dangerous assumption!)
    ...
    def get_containing_voxel_ijk(self, x, y, z):
        ...
        # print("get_containing_voxel_ijk: xx=",xx)
        # print("get_containing_voxel_ijk: xx.max()=",xx.max())
        # print("yy=",yy)
        # print("zz=",zz)
        ds = self.get_mesh_spacing()  # rwh: bad assumption about uniform spacing
        print("get_containing_voxel_ijk: ds=",ds)

        # rwh: comment out these confusing/bogus checks
        # if x > xx.max():
    ...
    def point_in_domain(self, x, y, z):  # rwh
        #print("_read_xml: bbox_coords= ",bbox_coords)  # rwh: [xmin ymin zmin xmax ymax zmax]
        if (x < self.bbox_coords[0]) or (x > self.bbox_coords[3]):
            warnings.warn('X= {0} out of bounds: x-range is {1}, {2})'.format(x, self.bbox_coords[0], self.bbox_coords[3]))
            sys.exit(-1)
        elif (y < self.bbox_coords[1]) or (y > self.bbox_coords[4]):
            warnings.warn('Y= {0} out of bounds: y-range is {1}, {2})'.format(y, self.bbox_coords[1], self.bbox_coords[4]))
            sys.exit(-1)
        elif (z < self.bbox_coords[2]) or (z > self.bbox_coords[5]):
            warnings.warn('Z= {0} out of bounds: z-range is {1}, {2})'.format(z, self.bbox_coords[2], self.bbox_coords[5]))
            sys.exit(-1)
    ...
    def get_concentrations_at(self, x, y, z):
        ...
        """
        self.point_in_domain(x,y,z)  # rwh
        i, j, k = self.get_containing_voxel_ijk(x, y, z)
        print("get_concentrations_at(): i,j,k=",i,j,k)
        ...
        for ix in range(len(sub_name_list)):
            ...
            print("get_concentrations_at(): concs[ix]=",concs[ix])
    ...
    def get_cell_df_at(self, x, y, z):
        ...
        """
        ds = self.get_mesh_spacing()  # rwh
        print("get_cell_df_at: ds = ",ds)
        xx, yy, zz = self.get_mesh()
        i, j, k = self.get_containing_voxel_ijk(x, y, z)
        x_vox = xx[j, i, k]
        y_vox = yy[j, i, k]
    ...
    def _read_xml(self, xml_file, output_path='.'):
        ...
        # while we're at it, find the mesh (rwh: the mesh of the *centers* of the voxels; not the domain mesh!)
        ...
        # rwh: also get bounding_box of domain to test for x,y,z being inside
        bbox_str = mesh_node.find('bounding_box').text
        self.bbox_coords = np.array(bbox_str.split(delimiter), dtype=np.float)
        print("_read_xml: self.bbox_coords= ",self.bbox_coords)  # rwh: [xmin ymin zmin xmax ymax zmax]
