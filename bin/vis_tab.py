    ...
    def plot_cell_scalar(self, frame):
        ...
        if cell_scalar_name in self.discrete_cell_scalars: 
            ...
            if cell_scalar_name == "current_phase":   # and if "Fixed" range is checked
                ...
                names_observed = [self.cycle_models[i] for i in self.discrete_variable_observed]

            elif cell_scalar_name == "current_death_model":
                self.discrete_variable = [0,1]
                names_observed = ["phase #%d" % i for i in self.discrete_variable_observed]
            
            elif cell_scalar_name == "is_motile":
                self.discrete_variable = [0,1]
                names_observed = ["motile" if i == 1 else "stationnary" for i in self.discrete_variable_observed]
                
            elif cell_scalar_name == "dead":
                self.discrete_variable = [0,1]
                names_observed = ["dead" if i == 1 else "alive" for i in self.discrete_variable_observed]
            else:
